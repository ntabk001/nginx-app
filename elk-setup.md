# Hướng dẫn Cài đặt ELK Stack từ File Tarball trên RHEL 8 với 3 Node Master và 3 Node Data + Ingest, Sử dụng Chứng chỉ Tự Sinh

## Giới thiệu
Hướng dẫn này cung cấp cách cài đặt ELK Stack (Elasticsearch, Logstash, Kibana) từ file tarball trên RHEL 8, với:
...
- Kibana và Logstash được cài trên một node riêng hoặc tích hợp vào một node data.
- Sử dụng chứng chỉ tự sinh (self-signed certificates) để kích hoạt bảo mật TLS/SSL.
...
- Mở firewall: Port 9200 (HTTP), 9300 (transport), 5601 (Kibana).

**Giả định hostname và IP** (thay đổi theo môi trường):
...
| Kibana/Logstash | kibana.example.com | 192.168.1.301   |
...
192.168.1.301 kibana.example.com
```
...
Thực hiện trên **tất cả node** cho Elasticsearch, và trên **node kibana** cho Kibana/Logstash.
...
3. Tải Logstash (trên node kibana):
   ```bash
...
5. Truy cập: https://192.168.1.301:5601.

## Bước 7: Kiểm tra và Bảo mật
- Kiểm tra cluster: `curl -u elastic:<pass> -k https://master1.example.com:9200/_cat/nodes?v`
- Cài Filebeat trên client để gửi log đến Logstash (port 5044).
- Tạo index pattern trong Kibana > Stack Management > Index Patterns.
